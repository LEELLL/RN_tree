...
parser.add_argument("-f","--feature_dim",type = int, default = 8)#64 resnet50裁剪版cut34\filter为２５６ #ｒｅｓｎｅｔ１８ 32 #resnet50 32 裁剪版layer34 裁剪版filter128#裁剪版filter64+cutlayer34

parser.add_argument("-r","--relation_dim",type = int, default = 8)
parser.add_argument("-w","--class_num",type = int, default = 5)# 类别数，即为n-way
parser.add_argument("-s","--sample_num_per_class",type = int, default = 5)#5  # 每个类别的样本数，即为k-shot
parser.add_argument("-b","--batch_num_per_class",type = int, default = 10)#10
parser.add_argument("-e","--episode",type = int, default= 500000)#500000 #1000
parser.add_argument("-t","--test_episode", type = int, default = 600)#600 #20
parser.add_argument("-i","--test_interval", type = int, default = 1000)#5000

parser.add_argument("-l","--learning_rate",type = float, default = 0.001)
parser.add_argument("-g","--gpu",type=int, default=0)
parser.add_argument("-u","--hidden_unit",type=int,default=10)
parser.add_argument("--model_name_or_path", default='lilingling/fewshot_apt/models_cut50/', type=str)
parser.add_argument("--datas_name_or_path", default='lilingling/miniimagenet2/', type=str)
...
class CNNEncoder(nn.Module):
    ...
    def forward(self,x):
        # print(x.size()) #torch.Size([25, 3, 84, 84])
        out = self.layer1(x)
        # print(out.size()) #torch.Size([25, 64, 41, 41])
        out = self.layer2(out)
        # print(out.size()) #torch.Size([25, 64, 19, 19])
        out = self.layer3(out)
        # print(out.size()) #torch.Size([25, 64, 19, 19])
        ...
        # print(out.shape)  ## torch.Size([25, 64, 19, 19])
        return out # 64
...
class PreTrainedResNet(nn.Module):
  ...
  def forward(self, x):
    x = self.resnet18 (x)
    # print('x:',x.shape)  # x: torch.Size([25, 512, 1, 1])   x: torch.Size([50, 512, 1, 1])
...
class PreTrainedResNet50(nn.Module):
  ...
  def forward(self, x):
    x = self.resnet50(x)
    # print('x:',x.shape)  # x: torch.Size([25, 2048, 1, 1])  x: torch.Size([50, 2048, 1, 1])
...
def main():
    ...
    # feature_encoder = PreTrainedResNet50()
    ...
    for episode in range(EPISODE):
        ...
        sample_features = sample_features.view(CLASS_NUM,SAMPLE_NUM_PER_CLASS,FEATURE_DIM,8,8) #resnet50 #resnet50裁剪layer34+filter为256 (16,8,8)
        # sample_features = sample_features.view(CLASS_NUM,SAMPLE_NUM_PER_CLASS,FEATURE_DIM,4,4) #resnet18 resnet50裁剪版 resnet50filter裁剪版
        ...
        relation_pairs = torch.cat((sample_features_ext,batch_features_ext),2).view(-1,FEATURE_DIM*2,8,8) #resnet50 #resnet50裁剪layer34+filter为256 (16,8,8)
        # relation_pairs = torch.cat((sample_features_ext,batch_features_ext),2).view(-1,FEATURE_DIM*2,4,4) #resnet18　　resnet50裁剪版  resnet50filter裁剪版
        ...
        if episode%TEST_INTERVAL == 0: #5000
            ...
            for i in range(TEST_EPISODE):
                ...
                for test_images,test_labels in test_dataloader:
                    ...
                    # print('batch_size', batch_size,test_labels.shape)
                    # calculate features
                    ...
                    relation_pairs = torch.cat((sample_features_ext,test_features_ext),2).view(-1,FEATURE_DIM*2,8,8) #resnet50 #resnet50裁剪layer34+filter为256 (16,8,8)
                    # relation_pairs = torch.cat((sample_features_ext,test_features_ext),2).view(-1,FEATURE_DIM*2,4,4)#resnet18　　resnet50裁剪版 resnet50filter裁剪版
                    relations = relation_network(relation_pairs).view(-1,CLASS_NUM)
            ...
            if test_accuracy > last_accuracy:
                # save networks
                # torch.save(feature_encoder.state_dict(),str(args.output_dir + "miniimagenet_feature_encoder_" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl"))
                # torch.save(feature_encoder.state_dict(),str(args.output_dir + "miniimagenet_relation_network_"+ str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl"))
