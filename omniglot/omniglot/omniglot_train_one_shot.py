...
import numpy as np
import task_generator_one as tg
import os
import math
import argparse
import random
...
parser.add_argument("--model_name_or_path", default='lilingling/fewshot_apt/orimodels/', type=str)
parser.add_argument("--datas_name_or_path", default='lilingling/omniglot_resized/', type=str)
...
class RelationNetwork(nn.Module):
    ...
    def __init__(self,input_size,hidden_size):
        ...
        logger.info("insize hiddensize",input_size,hidden_size)
        self.layer1 = nn.Sequential(
                        # nn.Conv2d(64*2,64,kernel_size=3,padding=0),
                        parse(FEATURE_DIM*2,FEATURE_DIM*2,stride=1),
        ...
        self.layer2 = nn.Sequential(
                        ...
                        nn.ReLU(),
                        nn.MaxPool2d(2))
        self.fc1 = nn.Linear(input_size*4*4*2,hidden_size)
    ...
    for episode in range(EPISODE):
        ...
        if (episode+1)%TEST_INTERVAL == 0:
            ...
            if test_accuracy > last_accuracy:
                ...
                # save networks
                modelname = str("omniglot_feature_encoder_" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")
                model_path = os.path.join(tracking.get_outputs_path(), modelname)
                torch.save(feature_encoder.state_dict(),model_path)


                modelname = str("omniglot_relation_network_" + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")
                model_path = os.path.join(tracking.get_outputs_path(), modelname)
